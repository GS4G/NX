...
class nx_lint_event(sublime_plugin.EventListener):
    ...
    def on_load(self, view):
        ...
        master_lint_enable = s.get("nx_master_lint", False)
        if self.is_vhdl(view) and lint_on_load and master_lint_enable:
    ...
    def on_modified(self, view):
        ...
        if view.file_name() is not None:
            if auto_lint and self.is_vhdl(view) and master_lint_enable:
...
class remove_linting(sublime_plugin.TextCommand):

    def run(self, edit):
        to_del = ["inst_", "p_", "g_", "b_", "constant", "variable", "signal", " type"]
        for region_to_del in to_del:
                try:
...
class coding_linting(sublime_plugin.TextCommand):
    ...
    def rm_regions_lint(self, region_key=[]):
